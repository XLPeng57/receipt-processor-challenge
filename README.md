...
```
git clone https://github.com/XLPeng57/receipt-processor-challenge.git
...
#### Submitting a Receipt using `curl`
...
There are four example JSON files inside the examples folder, each containing a receipt. You can replace `morning-receipt.json` with other JSON files.
...
To retrieve the points for a specific receipt, send a GET request to `/receipts/{id}/points`, replacing `{id}` with the ID of the receipt.

## About

Build a webservice that fulfils the documented API. The API is described below. A formal definition is provided in the [api.yml](./api.yml) file, but the information in this README is sufficient for completion of this challenge. We will use the described API to test your solution.
